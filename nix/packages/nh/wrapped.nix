...
let
  ...
  nh-launcher = fullPkgs.writeShellScriptBin "nh" ''
    set -euo pipefail
    usage() {
      cat >&2 <<'EOF'
nh — nohup helper
...
EOF
    ...
    latest_log() {
      ...
      local latest
      latest=$(${fullPkgs.coreutils}/bin/ls -1 "$outdir"/*.out 2>/dev/null | ${fullPkgs.coreutils}/bin/sort | ${fullPkgs.coreutils}/bin/tail -n 1 || true)
    ...
    if [ "$1" = "tail" ] || [ "$1" = "t" ]; then
      ...
      exec ${fullPkgs.coreutils}/bin/tail -n 100 -f "$latest"
    fi
    ...
    if [ "$1" = "ps" ]; then
      ...
      list_one() {
        ...
        for pf in "$pdir"/*; do
          [ -f "$pf" ] || continue
          pid=$(${fullPkgs.coreutils}/bin/basename "$pf")
          ...
          started=""
          [ -f "$pf" ] && started=$(${fullPkgs.coreutils}/bin/date -r "$pf" '+%Y-%m-%d %H:%M:%S' 2>/dev/null || true)
          printf '%-6s  %-8s  %s\n' "$pid" "$status" "$cmd"
        done
      }
      if [ "$#" -ge 1 ]; then
        ...
        for d in "$nh_root"/*; do
          [ -d "$d" ] || continue
          list_one "$(${fullPkgs.coreutils}/bin/basename "$d")"
        done
      fi
      exit 0
    fi
    ...
    if [ "$1" = "clean" ]; then
      ...
      clean_one() {
        ...
        if [ -d "$pdir" ]; then
          for pf in "$pdir"/*; do
            ...
            pid=$(${fullPkgs.coreutils}/bin/basename "$pf")
            if ! kill -0 "$pid" 2>/dev/null; then
              ${fullPkgs.coreutils}/bin/rm -f "$pf" || true
              cleaned=$((cleaned+1))
            fi
          done
      ...
      if [ "$#" -ge 1 ]; then
        ...
        for d in "$nh_root"/*; do
          [ -d "$d" ] || continue
          clean_one "$(${fullPkgs.coreutils}/bin/basename "$d")"
        done
      fi
      exit 0
    fi
    ...
    if [ "$1" = "prune-logs" ]; then
      ...
      prune_one() {
        ...
        local outdir="$state_home/nh/$cmd"
        if [ -d "$outdir" ]; then
          ${fullPkgs.findutils}/bin/find "$outdir" -maxdepth 1 -type f -name '*.out' -mtime "+$days" -print -delete 2>/dev/null || true
        fi
      }
      if [ "$#" -ge 1 ]; then
        prune_one "$1"
        ...
        for d in "$nh_root"/*; do
          [ -d "$d" ] || continue
          prune_one "$(${fullPkgs.coreutils}/bin/basename "$d")"
        done
      fi
      exit 0
    fi
    ...
    pid=$(cat "$outdir/pids"/* | ${fullPkgs.coreutils}/bin/tail -n1)
  ...
  wm = inputs.wrapper-manager.lib {
    ...
    modules = [
      {
        wrappers.nh = {
          basePackage = nh-launcher;
        };
      }
    ];
  };
in
wm.config.wrappers.nh.wrapped
